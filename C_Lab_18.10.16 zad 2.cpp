...
for (i=0; i<iMAXARRAY-1;i++)
{
	...
	if (!bSwap)
	{
	iArray_Out[iCount_Temp]=iArray_In[iMAXARRAY-1-i]; //если элемент не равен ни какому элементу в масиве тогда кладу его в выходной массив
	iCount_Temp--;										//счетчик заполнения выходного массива, так как выходной массив заполняется с конца идет на уменьшение
	}
}
